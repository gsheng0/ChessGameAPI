...
class MoveGeneratorServiceImplTest {
    ...
    public void KnightOnEmptyBoard(){
        ...
        testSinglePieceMoveGeneration(knight, 35, 18, 20, 25, 29, 41, 45, 50, 52);
        testSinglePieceMoveGeneration(knight, 49, 32, 34, 43, 59);
        testSinglePieceMoveGeneration(knight, 1, 11, 16, 18);
        testSinglePieceMoveGeneration(knight, 7, 13, 22);
        testSinglePieceMoveGeneration(knight, 39, 22, 29, 45, 54);
        testSinglePieceMoveGeneration(knight, 24, 9, 18, 34, 41);
        testSinglePieceMoveGeneration(knight, 61, 44, 46, 51, 55);
    }
    @Test
    public void BishopOnEmptyBoard(){
        Piece bishop = Piece.of('B');
        testSinglePieceMoveGeneration(bishop, 56, 49, 42, 35, 28, 21, 14, 7);
        testSinglePieceMoveGeneration(bishop, 63, 54, 45, 36, 27, 18, 9, 0);
        testSinglePieceMoveGeneration(bishop, 35, 56, 49, 42, 28, 21, 14, 7, 8, 17, 26, 44, 53, 62);
        testSinglePieceMoveGeneration(bishop, 36, 57, 50, 43, 29, 22, 15, 0, 9, 18, 27, 45, 54, 63);
        testSinglePieceMoveGeneration(bishop, 10, 1, 19, 28, 37, 46, 55, 3, 17, 24);
    }
    @Test
    public void RookOnEmptyBoard(){
        Piece rook = Piece.of('R');
        testSinglePieceMoveGeneration(rook, 56, 48, 40, 32, 24, 16, 8, 0, 57, 58, 59, 60, 61, 62, 63);
        testSinglePieceMoveGeneration(rook, 7, 6, 5, 4, 3, 2, 1, 0, 15, 23, 31, 39, 47, 55, 63);
        testSinglePieceMoveGeneration(rook, 36, 28, 20, 12, 4, 44, 52, 60, 32, 33, 34, 35, 37, 38, 39);
        testSinglePieceMoveGeneration(rook, 25, 24, 26, 27, 28, 29, 30, 31, 1, 9, 17, 33, 41, 49, 57);
        testSinglePieceMoveGeneration(rook, 54, 6, 14, 22, 30, 38, 46, 62, 48, 49, 50, 51, 52, 53, 55);
        testSinglePieceMoveGeneration(rook, 1, 0, 2, 3, 4, 5, 6, 7, 9, 17, 25, 33, 41, 49, 57);
    }
    @Test
    public void QueenOnEmptyBoard(){
        Piece queen = Piece.of('Q');
        testSinglePieceMoveGeneration(queen, 56, 48, 40, 32, 24, 16, 8, 0, 57, 58, 59, 60, 61, 62, 63,
                49, 42, 35, 28, 21, 14, 7);
        testSinglePieceMoveGeneration(queen, 36, 28, 20, 12, 4, 44, 52, 60, 32, 33, 34, 35, 37, 38, 39,
                57, 50, 43, 29, 22, 15, 0, 9, 18, 27, 45, 54, 63);
        testSinglePieceMoveGeneration(queen, 62, 6, 14, 22, 30, 38, 46, 54, 56, 57, 58, 59, 60, 61, 63,
                53, 44, 35, 26, 17, 8, 55);
        testSinglePieceMoveGeneration(queen, 11, 3, 19, 27, 35, 43, 51, 59, 8, 9, 10, 12, 13, 14, 15,
                2, 20, 29, 38, 47, 4, 18, 25, 32);
        testSinglePieceMoveGeneration(queen, 58, 50, 42, 34, 26, 18, 10, 2, 56, 57, 59, 60, 61, 62, 63,
                40, 49, 23, 30, 37, 44, 51);
    }
    @Test
    public void KingOnEmptyBoard(){
        Piece king = Piece.of('K');
        testSinglePieceMoveGeneration(king, 60, 51, 52, 53, 59, 61);
        testSinglePieceMoveGeneration(king, 27, 18, 19, 20, 26, 28, 34, 35, 36);
        testSinglePieceMoveGeneration(king, 39, 30, 31, 38, 46, 47);
        testSinglePieceMoveGeneration(king, 40, 32, 33, 41, 48, 49);
    }
    @Test
    ...
    private void testPinnedPieceMoveGeneration(int kingTile, String pinned, int pinnedTile, String pinning, int pinningTile, int... expected){
        ...
        Board board = boardBuilder.newBoard().withA(pinnedPiece).onTile(pinnedTile).withA(pinningPiece).onTile(pinningTile).withA(Piece.of("K")).onTile(kingTile).build();
